...
pub enum StorageServiceRequest {
    ...
    GetStateValueChunkWithProof(StateValueChunkWithProofRequest), // Fetches a list of states with a proof
...
impl StorageServiceRequest {
    ...
    pub fn get_label(&self) -> &'static str {
        match self {
            ...
            Self::GetStateValueChunkWithProof(_) => "get_state_value_chunk_with_proof",
...
#[derive(Clone, Debug, Deserialize, Eq, Hash, PartialEq, Serialize)]
pub struct StateValueChunkWithProofRequest {
...
pub struct ProtocolMetadata {
    ...
    pub max_state_value_chunk_size: u64, // The max number of states the server can return in a single chunk
}
...
impl ProtocolMetadata {
    ...
    pub fn can_service(&self, request: &StorageServiceRequest) -> bool {
        ...
        match request {
            ...
            GetStateValueChunkWithProof(request) => CompleteDataRange::new(
                request.start_index,
                request.end_index,
            )
            .map_or(false, |range| {
                range.len().map_or(false, |chunk_size| {
                    self.max_state_value_chunk_size >= chunk_size
                })
            }),
...
impl Default for ProtocolMetadata {
    fn default() -> Self {
        ...
        Self {
            ...
            max_state_value_chunk_size: config.max_state_chunk_size,
        }
    }
}
...
impl DataSummary {
    ...
    pub fn can_service(&self, request: &StorageServiceRequest) -> bool {
        ...
        match request {
            ...
            | GetServerProtocolVersion
            | GetStorageServerSummary => true,
            GetStateValueChunkWithProof(request) => {
...
mod tests {
    ...
    fn get_states_request(version: Version) -> StorageServiceRequest {
        get_state_chunks_request(version, 0, 1000)
    }
    ...
    fn test_protocol_metadata_can_service() {
        let metadata = ProtocolMetadata {
            ...
            max_state_value_chunk_size: 100,
        };
        ...
        assert!(metadata.can_service(&get_state_chunks_request(200, 100, 199)));
        assert!(!metadata.can_service(&get_state_chunks_request(200, 100, 200)));
    }
